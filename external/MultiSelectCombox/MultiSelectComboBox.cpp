                                                             ...
                                                             mSearchBar(new QLineEdit(this))
{
    ...
    // 下拉子项
    mLineEdit->setReadOnly(true);        // 设置只读
    mLineEdit->installEventFilter(this); // 安装事件过滤器
    setModel(mListWidget->model());
    setView(mListWidget);
    setLineEdit(mLineEdit); // 设置下拉项
    mLineEdit->setStyleSheet("QLineEdit:disabled{background:rgb(233,233,233);}");

    // 搜索框文本触发
    connect(mSearchBar, SIGNAL(QLineEdit::textChanged(QString &)), this, SLOT(MultiSelectComboBox::onSearch(QString &)));
