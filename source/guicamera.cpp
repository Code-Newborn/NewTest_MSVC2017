...
CGuiCamera::CGuiCamera() : m_cntGrabbedImages(0), m_cntSkippedImages(0), m_cntGrabErrors(0), m_cntLastGrabbedImages(0), m_userHint(-1), m_invertImage(false)
{
...
void CGuiCamera::OnImageGrabbed(Pylon::CInstantCamera &camera, const Pylon::CGrabResultPtr &grabResult)
{
    ...
    if (grabResult.IsValid() && grabResult->GrabSucceeded())
    {
        ...
        if ((static_cast<uint32_t>(m_image.width()) != grabResult->GetWidth()) || (static_cast<uint32_t>(m_image.height()) != grabResult->GetHeight()))
        {
